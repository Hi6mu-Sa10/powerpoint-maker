...
```json
{
  "slides": [
    ...
    // 箇条書きコンテンツ
    {
      ...
      "title": "スライドタイトル",
      "bullets": [
        {"text": "メインポイント", "sub": "補足説明（省略可）", "ref": 1},
        {"text": "メインポイント2"}
      ],
      "note": "補足メモ（省略可）"
    },
    ...
    // 参考文献（web_contextが提供された場合は必須・クロージング前）
    {
      "type": "references",
      "title": "参考文献",
      "items": [
        {"number": 1, "title": "参照元のタイトル", "url": "https://example.com", "accessed": "2025年5月"},
        {"number": 2, "title": "参照元のタイトル2", "url": "https://example2.com", "accessed": "2025年5月"}
      ]
    },
    // クロージング（必須・最後）
    {
...
- web_contextが提供された場合は必ずその情報を根拠にコンテンツを作成し、参照元番号（ref）をbulletsに付与すること
- web_contextが提供された場合は必ずreferencesスライドを追加し、使用した参照元のみ記載すること
- web_contextが提供されない場合はreferencesスライドを追加しないこと
- 情報源が不明な統計・数値・事例は記載しないこと
"""
...
def generate_slides(
    ...
    Args:
        ...
        web_context: 事前に収集したWeb情報のリスト。各要素は
                     {"number": 1, "title": "...", "url": "...", "summary": "..."} 形式
    ...
    # web_contextが提供された場合はプロンプトに追加
    if web_context:
        parts.append("\n## 参照情報（以下の情報のみを根拠としてコンテンツを作成してください）")
        parts.append("以下の情報源から得られた事実のみを使用し、情報源が不明な統計・数値・事例は記載しないでください。")
        parts.append("各bulletには使用した参照元の番号（ref）を必ず付与してください。")
        parts.append("")
        for item in web_context:
            ...
            parts.append("")
        parts.append("上記の参照情報をもとに、referencesスライドをクロージング前に追加してください。")
    ...
    if not slides:
        msg = "スライドデータが空です。Bedrockからの応答を確認してください。"
